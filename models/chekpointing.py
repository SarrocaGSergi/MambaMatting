...
class MattenEncoder(nn.Module):
    def __init__(self, patch_size=2, in_chans=3, depths=[2, 2, 9, 2], dims=[96, 192, 384, 768],
                 d_state=16, attn_drop_rate=0., norm_layer=nn.LayerNorm, use_checkpoint=True, **kwargs):
        super().__init__()
    ...
    def _init_weights(self, m: nn.Module):
        """
        out_proj.weight which is previously initialized in VSSBlock, would be cleared in nn.Linear
        no fc.weight found in any of the model parameters
        no nn.Embedding found in any of the model parameters
    ...
    def forward(self, x):
        ...
        for s, layer in enumerate(self.layers):
            if self.use_checkpoint:
                z_token, z_proj = checkpoint(layer, z_token, nf, nh, nw, use_reentrant=False)
            else:
...
class MatMat(nn.Module):
    def __init__(self, patch_size, in_channels=3, out_chans=1, feat_size=[48, 96, 192, 384, 768], drop_path_rate=0,
        ...
        self.encoders = nn.ModuleList([
            UnetrBasicBlock(spatial_dims=spatial_dims, in_channels=self.in_channels, out_channels=self.feat_size[0], kernel_size=3, stride=1, norm_name=norm_name, res_block=res_block),
            UnetrBasicBlock(spatial_dims=spatial_dims, in_channels=self.feat_size[0], out_channels=self.feat_size[1], kernel_size=3, stride=1, norm_name=norm_name, res_block=res_block),
            UnetrBasicBlock(spatial_dims=spatial_dims, in_channels=self.feat_size[1], out_channels=self.feat_size[2], kernel_size=3, stride=1, norm_name=norm_name, res_block=res_block),
            UnetrBasicBlock(spatial_dims=spatial_dims, in_channels=self.feat_size[2], out_channels=self.feat_size[3], kernel_size=3, stride=1, norm_name=norm_name, res_block=res_block),
            UnetrBasicBlock(spatial_dims=spatial_dims, in_channels=self.feat_size[3], out_channels=self.feat_size[4], kernel_size=3, stride=1, norm_name=norm_name, res_block=res_block),
            UnetrBasicBlock(spatial_dims=spatial_dims, in_channels=hidden_size, out_channels=self.feat_size[4], kernel_size=3, stride=1, norm_name=norm_name, res_block=res_block),
        ])

        self.decoders = nn.ModuleList([
            UnetrUpBlock(spatial_dims=spatial_dims, in_channels=self.hidden_size, out_channels=self.feat_size[4],  kernel_size=3, upsample_kernel_size=2, norm_name=norm_name, res_block=res_block),
            UnetrUpBlock(spatial_dims=spatial_dims, in_channels=self.feat_size[4], out_channels=self.feat_size[3], kernel_size=3, upsample_kernel_size=2, norm_name=norm_name, res_block=res_block),
            UnetrUpBlock(spatial_dims=spatial_dims, in_channels=self.feat_size[3], out_channels=self.feat_size[2], kernel_size=3, upsample_kernel_size=2, norm_name=norm_name, res_block=res_block),
            UnetrUpBlock(spatial_dims=spatial_dims, in_channels=self.feat_size[2], out_channels=self.feat_size[1], kernel_size=3, upsample_kernel_size=2, norm_name=norm_name, res_block=res_block),
            UnetrUpBlock(spatial_dims=spatial_dims, in_channels=self.feat_size[1], out_channels=self.feat_size[0], kernel_size=3, upsample_kernel_size=2, norm_name=norm_name, res_block=res_block),
            UnetrBasicBlock(spatial_dims=spatial_dims, in_channels=self.feat_size[0], out_channels=self.feat_size[0], kernel_size=3, stride=1, norm_name=norm_name, res_block=res_block)
        ])
        ...
        if self.deep_supervision:
            ...
            self.matting_out_layer = UnetOutBlock(spatial_dims=spatial_dims, in_channels=self.feat_size[0], out_channels=4)

        self.seg_out_layer = UnetOutBlock(spatial_dims=spatial_dims, in_channels=self.feat_size[0], out_channels=1)
    ...
    def forward(self, x, seg_pass = False):
        ...
        def run_layers(layer, *inputs):
            if self.use_checkpoint:
                return checkpoint(layer, *inputs, use_reentrant=False)
            else:
                return layer(input)

        enc1 = run_layers(self.encoders[0], x_spat)
        enc2 = run_layers(self.encoders[1], vss_outs[0])
        enc3 = run_layers(self.encoders[2], vss_outs[1])
        enc4 = run_layers(self.encoders[3], vss_outs[2])
        enc5 = run_layers(self.encoders[4], vss_outs[3])
        enc_hidden = vss_outs[4]

        dec4 = run_layers(self.decoders[0], enc_hidden, enc5)
        dec3 = run_layers(self.decoders[1], dec4, enc4)
        dec2 = run_layers(self.decoders[2], dec3, enc3)
        dec1 = run_layers(self.decoders[3], dec2, enc2)
        dec0 = run_layers(self.decoders[4], dec1, enc1)
        dec_out = self.decoders[5](dec0)

        if self.deep_supervision and not seg_pass and self.matting_out_layers:
        ...
        elif not self.deep_supervision and not seg_pass and self.matting_out_layer:
            out = []
            pred = self.matting_out_layer(dec_out)
...
if __name__ == '__main__':
    x = torch.randn(2, 2, 3, 224, 512).to("cuda")
    model = MatMat(patch_size=2, feat_size=[48, 96, 192, 384, 768], use_checkpoint=True).to("cuda")
    print(model)
    pred = model(x)
    print(pred)