...
class VSSMEncoder(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, depths=[2, 2, 9, 2],
        ...
        self.timestep_embed = TimestepEmbedder(hidden_size=self.embed_dim)
...
class SwinUMamba(nn.Module):
    def __init__(self, in_chans=3, out_chans=13, feat_size=[48, 96, 192, 384, 768], drop_path_rate=0,
                 ...
                 spatial_dims=2, deep_supervision: bool = False) -> None:
        super().__init__()
...
class Tokenizer3D(nn.Module):
    def __init__(self, patch_size, image_size):
        super(Tokenizer3D, self).__init__()
        self.patch_size = patch_size
        self.image_size = image_size
        self.patch_embedder = PatchEmbed3D(patch_size, image_size)
...
if __name__ == "__main__":
    ...
    x = torch.rand(1, 3, 3, 1024, 2048).to("cuda")
    model = PatchEmbed3D(patch_size=8, embed_dim=96).cuda()
    mamba_layer = Mamba(
        # This module uses roughly 3 * expand * d_model^2 parameters
        d_model=96,         # Model dimension d_model
        d_state=16,         # SSM state expansion factor
        d_conv=4,           # Local convolution width
        expand=2,           # Block expansion factor
        use_fast_path=False,
        ).cuda()
    attn = Attention(96, num_heads=8, attn_drop=0., proj_drop=0., attention_mode='flash').cuda()
    pred = model(x)
    B, emb_d, nf, nh, nw = pred.shape
    z_prim = pred.view(B, emb_d, nh * nw *nf).permute(0, 2, 1)
    z_out = mamba_layer(z_prim)
    # Restructure to compute spatial-attention
    z_spatial = z_out.view(B*nf, nh*nw, emb_d)
    z_spattn = attn(z_spatial)
    # Restructure to compute temporal-attention
    z_temp = z_spattn.view(B*nh*nw, nf, emb_d)
    z_attn = attn(z_temp)
    x = 2
